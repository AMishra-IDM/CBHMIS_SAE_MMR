...
model_no <- 4 ### Replace with current model run
...
combined_df <- left_join(summary_df,data[,c("LGA","avgRepRate","lb","deaths","MMR","anyEd",
                                           "tt_mean_unweighted","ANC.1st")])
combined_df$repRate_scaled <- nimble_data$repRate
...
pRepPred <- ggplot(combined_df, aes(x = repRate_scaled, y = pi_median)) +
              geom_point() +
