...
summary(modData$avgRepRate)
hist(modData$avgRepRate)
reporting_covars <- scale(modData[, "avgRepRate"],
                          center = TRUE, scale = TRUE)
summary(reporting_covars)
hist(reporting_covars)

ggplot(modData, aes(x = avgRepRate, y = reporting_covars[,1], label = LGA)) +
  geom_point() +
  geom_text_repel(size = 3) +
  theme_minimal()

## just checking direction of data 
glm(cbind(modData$deaths, modData$lb - modData$deaths) ~ modData$avgRepRate, family = binomial)
