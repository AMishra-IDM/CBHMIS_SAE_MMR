...
cbhmis_lga <- cbhmis[,!names(cbhmis) %in% "Ward"] %>% group_by(Month, LGA) %>% summarise(across(everything(), sum))
cbhmis_lga_agg <- cbhmis_lga[,names(cbhmis_lga) %in% c("LGA","Live.births","Still.births","Live.births","Maternal.deaths","Neonatal.death","Under.1.death","Under.5.death")] %>% 
  group_by(LGA) %>% summarise(across(everything(), sum))
